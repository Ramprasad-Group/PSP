...
def OBmolConfSearchRDkit(unitname, mol, work_dir, rdkit_conf_param, Nconf=1, NCores_opt=0, seed=None, NAttempt=10000, BasicKnowledge=True, RmsThresh=0.1, ExpTorsionAnglePref=True, enforceChirality=False):
    ...
    # Set parameters
    if 'numConfs' in rdkit_conf_param[0].keys():
        Nconf = rdkit_conf_param[0]['numConfs']

    if 'numThreads' in rdkit_conf_param[0].keys():
        NCores_opt = rdkit_conf_param[0]['numThreads']

    if 'randomSeed' in rdkit_conf_param[0].keys():
        seed = rdkit_conf_param[0]['randomSeed']

    if 'maxAttempts' in rdkit_conf_param[0].keys():
        NAttempt = rdkit_conf_param[0]['maxAttempts']

    if 'useBasicKnowledge' in rdkit_conf_param[0].keys():
        BasicKnowledge = rdkit_conf_param[0]['useBasicKnowledge']

    if 'pruneRmsThresh' in rdkit_conf_param[0].keys():
        RmsThresh = rdkit_conf_param[0]['pruneRmsThresh']

    if 'useExpTorsionAnglePrefs' in rdkit_conf_param[0].keys():
        ExpTorsionAnglePref = rdkit_conf_param[0]['useExpTorsionAnglePrefs']

    if 'enforceChirality' in rdkit_conf_param[0].keys():
        enforceChirality = rdkit_conf_param[0]['enforceChirality']
    ...
    path_ini_geo = os.path.join(work_dir, unitname + '.mol')
    obConversion.WriteFile(mol, path_ini_geo)
    ...
    if m2 is not None:
        ...
        for cid in cids:
            n += 1
            path_out_geo = os.path.join(work_dir, unitname + 'conf_' + str(n) + '.xyz')
            AllChem.UFFOptimizeMolecule(m2, confId=cid)
            Chem.MolToXYZFile(m2, path_out_geo, confId=cid)

            # Read each conformer as OBMol
            mol = ob.OBMol()
            obConversion.ReadFile(mol, path_out_geo)
            list_conf.append(mol)

            # Exit the function if you have desired number of conformers
            if n == Nconf:
                return list_conf
    return list_conf
...
def GenOutput(
    ...
    if any(i in POSCAR_list for i in list_output):
        out_filename = os.path.join(output_dir, unitname + '_C' + str(n) + '.vasp')
    ...
    if any(i in LAMMPS_list for i in list_output):
        out_filename = os.path.join(output_dir, unitname + '_C' + str(n) + '.lmp')
        MDlib.gen_sys_data(
            out_filename,
            unit,
            "",
    ...
    if any(i in GAFF_list for i in list_output):
        out_filename = os.path.join(output_dir, unitname + '_C' + str(n))
    ...
    if any(i in OPLS_list for i in list_output):
        gen_opls_data(unitname, OBMol, output_dir, unitname, n)
    ...
    GenOutputOB(unitname, OBMol, list_output, output_dir, n)
...
def gen_opls_data(unit_name, OBMol, output_dir, unitname, n):
    ...
    print(unit_name, "Conformer - ", n, ": Generating OPLS parameter file ...")
    out_filename = os.path.join(output_dir, unitname + '_C' + str(n) + '_opls')
    try:
        Converter.convert(
